...
public class SportsSystem {
	...
	public ArrayList<String> practicedSportsBuffer;
	public ArrayList<String> interestSportsBuffer;

	SportsSystem() {
		students = new AVL<>();
		sports   = new AVL<>();
		practicedSportsBuffer = new ArrayList<String>();
		interestSportsBuffer = new ArrayList<String>();
	}


	public void createStudent(String name, int id, boolean bufferedSports) {
		...
		if (bufferedSports)
		{
			...
			for (int i = 0; i < practicedSportsBuffer.size(); i++)
			{
				String input = practicedSportsBuffer.get(i);
			...
			for (int i = 0; i < interestSportsBuffer.size(); i++)
			{
				String input = interestSportsBuffer.get(i);
				Sport found = getSport(input);
				if (found == null) {
					System.out.println("  Sport not found: " + input);
				} else {
	...
	public void removeSportFromStudent(int id, String sportName) {
		...
		sport.removePracticerByRef(ref);   // O(1) DLL removal
		stu.removePractice(sport, ref);    // O(k) SLL scan — k is tiny
		stu.removeInterest(sport);
	...
	public void printSportsByCount() {
		DinamicArray<Sport> arr = new DinamicArray<Sport>();
